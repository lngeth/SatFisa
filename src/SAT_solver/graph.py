...
class GraphSAT:
  ...
  Attributes:
    ...
    nb_color: Number of color to draw the Graph (initially 3, and max 4)
  """

  def __init__(self, graph_s):
    """Init the Graph object with String format Graph given by user
      graph_s: the Graph in string format
    """
  ...
  def get_dimacs_clauses(self, export_to=""):
    """Convert the Graph object into Dimacs format
      ...
      -> Return a tuple (head, list_clauses)
    """
    ...
    list_clause = [] # list of clauses to return
  ...
  def get_Ncolor_graph(self, sat_res):
    """Get the color of each node
      sat_res: the SAT result given by Glucose
      -> Return the list of variables with the corresponding color
    """
